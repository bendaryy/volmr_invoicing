    ...
    <ul class="nav align-items-center">
        ...
        <li class="px-sm-3 px-2">
            @if(\Illuminate\Support\Facades\Auth::user()->dark_mode)
                <a href="{{ route('update-dark-mode') }}"><i class="fa-solid fa-moon text-primary fs-2"></i></a>
            @else
                <a href="{{ route('update-dark-mode') }}"><i class="fa-solid fas fa-sun text-primary fs-2"></i></a>
            @endif
        </li>
        <li class="px-sm-3 px-2">
            <div class="dropdown custom-dropdown d-flex align-items-center">
                <button class="btn dropdown-toggle hide-arrow position-relative d-flex align-items-center hoverable" type="button" id="dropdownMenuButton1"
                        data-bs-toggle="dropdown" aria-expanded="false">
                    <i class="fa-solid fa-bell text-primary fs-2"></i>
                    @if(count(getNotification()) != 0)
                        <span class="badge navbar-badge bg-primary notification-count notification-message-counter rounded-circle position-absolute translate-middle d-flex justify-content-center align-items-center {{($notificationCount > 9)?'end-0':'counter-0'}}" {{$styleCss}}="top:8px"
